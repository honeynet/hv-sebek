...
static void setup_i8259(void)
{
	...
	printf("Exit setup_i8259\n");
}

static void hardware_setup(struct param_info *info __unused)
{
	printf("hardware_setup\n");
	/* Disable nmi */
	outb(0x80, 0x70);	//output byte 0x80 to port 0x70
...
static void query_lb_values(struct param_info *info)
{
	...
	head = info->lb_table;
	start = ((unsigned char *)head) + sizeof(*head);	//skipping the header
...
static void get_meminfo(struct param_info *info)
{
	...
	meminfo.map_count = meme820(meminfo.map, E820MAX);		//Anh - die here
...
static void query_bootloader_param_class(struct param_info *info)
{
	...
	printf("Exit query_bootloader_param_class\n");
}

static void query_bootloader_values(struct param_info *info) 
{
	printf("query_bootloader_values\n");
	
	if (info->has_multiboot) {
		printf("Has multiboot\n");
	...
	printf("Exit query_bootloader_values\n");
}
...
{
	printf("query_firmware_values\n");
	
	if (info->has_linuxbios) {
		printf("has_linuxbios\n");
	...
	printf("Exit query_firmware_values\n");	
}
...
void initialize_linux_params(struct param_info *info)
{
	int len;
	/* First the defaults */
	memset(info->real_mode, 0, PAGE_SIZE);	//Anh - 0x20000
	...
	info->real_mode->cl_offset = 2048;		//Anh - info->real_mode->command_line is at offset 2048 (0x800)

	info->real_mode->cmd_line_ptr = info->real_mode->cl_offset + (unsigned long) info->real_mode;
	
	/* Now set the command line */
	//TODO! - Anh - check the vaule of info->image->cmdline 
...
void breakpoint(const char * szMsg)
{
	char * sz = (char *) 0x00ffffff;   
	printf(szMsg);
...
void *convert_params(unsigned type, void *data, void *param, void *image)
{
	...
	info.real_mode = faked_real_mode;	//Anh - faked_real_mode = 0x20000 = 128KB
	info.type  = type;
	info.data  = data;
	info.param = param;
	info.image = image;
