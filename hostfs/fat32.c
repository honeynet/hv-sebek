...
int read_sector(unsigned char *data, int dos_sector) {
//	dos_sector = (sector - 1) + (head * sectors_per_track) + (track * sectors_per_track * num_heads)
...
int write_sector(unsigned char *data, int blocknum) {
...
int tf_init() {
	...
	tf_info.totalSectors = (bpb->TotalSectors16 != 0) ? bpb->TotalSectors16
			: bpb->TotalSectors32;
	data_sectors = tf_info.totalSectors - (bpb->ReservedSectorCount
			+ (bpb->NumFATs * fat_size) + root_dir_sectors);
	...
	tf_info.firstDataSector = bpb->ReservedSectorCount + (bpb->NumFATs
			* fat_size) + root_dir_sectors;
...
int tf_first_sector(int cluster) {
	return ((cluster - 2) * tf_info.sectorsPerCluster)
...
char *tf_walk(char *filename, TFFile *fp) {
	...
	if (*filename != '\x00') {
		...
		fp->startCluster = ((entry.msdos.eaIndex & 0xffff) << 16)
		...
		fp->size = (entry.msdos.attributes & TF_ATTR_DIRECTORY) ? 0xffffffff
...
int tf_place_lfn_chain(TFFile *fp, char *filename) {
	...
	for (i = 0; i < entries; i++) {
		...
		entry.lfn.checksum = tf_lfn_checksum((unsigned char*) sfn);
#ifdef TF_DEBUG
...
#ifdef TF_DEBUG
	printf(
			"[DEBUG] Comparing filename segment '%s' (given) to '%s' (from disk)\n",
	...
	if (entry.msdos.attributes != 0x0f) {
		...
		for (i = 0; i < lfn_entries; i++) {
			...
			if (!tf_compare_filename_segment(&entry, compare_name)) {
				tf_fseek(fp, ((lfn_entries - i - 1)) * sizeof(FatFileEntry),
						fp->pos);
				return 0;
			}
			tf_fseek(fp, -sizeof(FatFileEntry), fp->pos);
...
int tf_fwrite(void *src, int size, int count, TFFile *fp) {
	...
	while (count > 0) {
		i = size;
		while (i > 0) {
			tf_fetch(tf_first_sector(fp->currentCluster) + (fp->currentByte
					/ 512));
...
#ifdef TF_DEBUG
			printf(
					"[DEBUG] Opened %s's parent for directory entry modification...\n",
					fp->filename);
#endif
